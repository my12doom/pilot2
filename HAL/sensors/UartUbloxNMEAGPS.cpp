...
// 		printf("packet:%2x, %2x, CRC(%s)\n", p->cls, p->id, p->crc_ok ? "OK" : "FAIL");
		...
		if (p->crc_ok)
		{
			...
			if (p->cls == 0x1 && p->id == 0x7)
			{
				...
				local_data.direction = atan2((float)pvt->velocity[1], (float)pvt->velocity[0]) * 180 / PI + 180;			// Track angle in degrees True, 0-360 degree, 0: north, 90: east, 180: south, 270: west, 359:almost north
