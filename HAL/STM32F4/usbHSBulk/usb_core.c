                               ...
                               USB_OTG_CORE_ID_TypeDef coreID)
{
	...
	/* at startup the core is in FS mode */		// 启动时，USB核处于全速状态
	pdev->cfg.speed            = USB_OTG_SPEED_FULL;		// 在usb_defines.h中定义为1，HS为0
	pdev->cfg.mps              = USB_OTG_FS_MAX_PACKET_SIZE ;    	// 在usb_regs.h中定义为64bytes，高速为512bytes
	...
	{
		baseAddress                = USB_OTG_HS_BASE_ADDR;		// 在usb_regs.h中定义为：0x40040000
		pdev->cfg.coreID           = USB_OTG_HS_CORE_ID;
		pdev->cfg.host_channels    = 12 ;						// 12个pipe，每个EP有进有出?
		pdev->cfg.dev_endpoints    = 6 ;						// 高速模式有6个EP
	...
	// 全速寄存器在AHB2时钟区域实现，高速在AHB1实现
	// 高速请参考reference manual的page1085
	/* 这里其实是定义了Core global CSRs寄存器组的基址 */
	pdev->regs.GREGS = (USB_OTG_GREGS *)(baseAddress + \
	                                     USB_OTG_CORE_GLOBAL_REGS_OFFSET);		// 在usb_regs.h定义为0x000
	/* 这里其实是定义了Device mode CSRs寄存器组的基址 */
...
uint32_t USB_OTG_GetEPStatus(USB_OTG_CORE_HANDLE *pdev ,USB_OTG_EP *ep)
{
	...
	if (ep->is_in == 1)
	{
		depctl_addr = &(pdev->regs.INEP_REGS[ep->num]->DIEPCTL);		// refer to page1140 (reference manual)
		depctl.d32 = USB_OTG_READ_REG32(depctl_addr);
