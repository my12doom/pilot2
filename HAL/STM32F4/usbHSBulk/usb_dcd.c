...
// USB_OTG_EP , *PUSB_OTG_EP;
	...
	USB_OTG_SelectCore (pdev , coreID);			// 位于usb_core.c，根据ID号选择高速还是全速,并定义了模式所能使用的端点数，寄存器组的基址等内容

	pdev->dev.device_status = USB_OTG_DEFAULT;	// USB定义了四个状态：默认，地址，配置，挂起，这里设置为默认先
	pdev->dev.device_address = 0;					// 未枚举之前，设备地址为0

	/* Init ep structure */		// 初始化端点结构体
	for (i = 0; i < pdev->cfg.dev_endpoints ; i++)		//pdev->cfg.dev_endpoints在USB_OTG_SelectCore函数中赋值，高速为6，全速为4
	{
		ep = &pdev->dev.in_ep[i];
		/* Init ep structure */
		ep->is_in = 1;
		...
		ep->type = EP_TYPE_CTRL;				// 就是指usb的四种传输方式，这里是控制传输
	...
	USB_OTG_DisableGlobalInt(pdev);		// 屏蔽USB中断

	/*Init the Core (common init.) */
	USB_OTG_CoreInit(pdev);				// 初始化USB核


	/* Force Device Mode*/
	USB_OTG_SetCurrentMode(pdev, DEVICE_MODE);		// 设置为设备模式

	/* Init Device */
	USB_OTG_CoreInitDev(pdev);	// 设备模式初始化
                     ...
                     uint8_t ep_type)
{
	...
	if ((ep_addr & 0x80) == 0x80)			// bit7=1, IN EP; bit7=0, OUT EP
	{
...
{
	USB_OTG_EP *ep;
	if ((0x80 & epnum) == 0x80)			//D7=1->device to host(IN EP)
	{
