...
{
	int available = VCP_available();
	if (available == 0)
		return 0;
	if (maxcount > available)
		maxcount = available;
	
	int linesize = 0;
	for(int i=0; i<maxcount; i++)
	{
		if (APP_Tx_Buffer[(APP_tx_ptr_tail+i)%APP_TX_BUF_SIZE] == '\n')
		{
			linesize = i+1;
			break;
		}
	...
	{
		buf[i] = APP_Tx_Buffer[(APP_tx_ptr_tail+i)%APP_TX_BUF_SIZE];	
	}
	
	APP_tx_ptr_tail = (APP_tx_ptr_tail+linesize)%APP_TX_BUF_SIZE;
	return linesize;
}
...
int VCP_get_data(uint8_t *buf, int maxcount)
{
	...
	APP_tx_ptr_tail = (APP_tx_ptr_tail+maxcount)%APP_TX_BUF_SIZE;
...
int VCP_available()
{
	...
	if (APP_tx_ptr_tail > APP_tx_ptr_head)
		return APP_tx_ptr_head + APP_TX_BUF_SIZE - APP_tx_ptr_tail;
	else
