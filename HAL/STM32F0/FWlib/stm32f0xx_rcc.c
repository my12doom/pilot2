...
/* CR register byte 2 (Bits[23:16]) base address 时钟控制寄存器*/
#define CR_BYTE2_ADDRESS          ((uint32_t)0x40021002)

/* CFGR register byte 3 (Bits[31:23]) base address时钟配置寄存器 */
#define CFGR_BYTE3_ADDRESS        ((uint32_t)0x40021007)

/* CIR register byte 1 (Bits[15:8]) base address时钟中断寄存器 */
#define CIR_BYTE1_ADDRESS         ((uint32_t)0x40021009)

/* CIR register byte 2 (Bits[23:16]) base address时钟中断寄存器 */
...
{
  /* Set HSION bit 开内部高速时钟*/
  RCC->CR |= (uint32_t)0x00000001;

  /* 时钟配置寄存器Reset SW[1:0], HPRE[3:0], PPRE[2:0], ADCPRE and MCOSEL[2:0] bits */
	//MCO关：000; PLL输入时钟X16;高速外部时钟被PLL/2;ADC时钟/2;PCLK,HCLK无分频；软件选择HSI作为系统时钟；
...
/**
  * @brief  Waits for HSE start-up.等待HSE开始
...
/**
  * @brief  Adjusts the Internal High Speed oscillator (HSI) calibration value.改变内部震荡时钟的值
...
/**
  * @brief  Enables or disables the Internal High Speed oscillator (HSI).开关内部高速时钟
...
/**
  ...
  *         calibration value.设置内部高速时钟对ADC
  * @note   The calibration is used to compensate for the variations in voltage
  *         and temperature that influence the frequency of the internal HSI RC.
  *         Refer to the Application Note AN3300 for more details on how to  
  *         calibrate the HSI14.
...
/**
  * @brief  Enables or disables the Internal High Speed oscillator for ADC (HSI14).使能内部高速时钟
