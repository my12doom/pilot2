...
using namespace HAL;
static STM32F1::F1Interrupt *int_table[16] = {0};
...
{	
	...
	bool F1Interrupt::init(GPIO_TypeDef* GPIOx, uint32_t GPIO_Pin, int flag)
	{
		this->GPIOx = GPIOx;
	...
	int F1Interrupt::enable()
	{
		NVIC_EnableIRQ(pin2irqn(GPIO_Pin));
		return 0;
	}

	int F1Interrupt::disable()
	{
		NVIC_DisableIRQ(pin2irqn(GPIO_Pin));
		return 0;
	}
	
	void F1Interrupt::set_callback(HAL::interrupt_callback cb, void *parameter)
	{
		this->cb=cb;
		this->parameter = parameter;
	}
	
	void F1Interrupt::call_callback()
	{
		if(cb)
			cb(parameter, flag);
	}
