...
static esp_loader_error_t SLIP_send(const uint8_t *data, uint32_t size)
{
	...
	{
		if (data[i] == 0xc0)
			serial_write(C0_REPLACEMENT, 2);
		else if (data[i] == 0xdb)
			serial_write(DB_REPLACEMENT, 2);
		else
			serial_write(&data[i], 1);
	}
...
esp_loader_error_t loader_memory_begin_cmd(uint32_t offset,
	uint32_t total_size,
	uint32_t block_size,
	uint32_t blocks_to_write)
{
...
esp_loader_error_t loader_memory_data_cmd(const uint8_t *data, uint32_t size)
{
	data_command_t data_cmd = {
		.common = {
			.direction = WRITE_DIRECTION,
...
esp_loader_error_t loader_memory_end_cmd(uint32_t run_flag, uint32_t entry_point)
{
...
esp_loader_error_t loader_change_baudrate_cmd(uint32_t baudrate, uint32_t old_baud)
{
    change_baudrate_command_t baudrate_cmd = {
        ...
        .old_baudrate = old_baud // ESP32 ROM only
    };
