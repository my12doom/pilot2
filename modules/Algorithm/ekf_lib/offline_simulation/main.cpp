...
typedef struct{
	float time,gx,gy,gz,ax,ay,az,mx,my,mz,gpsn,gpse,gpsd,gpsvn,gpsve,baro,roll,pitch,yaw;
}log_type;
int main()
{
	ekf_estimator ekf_estimator;
	cout<<"################ EKF offline simulation ################"<<endl;
	FILE* fp=fopen("Z:\\fusing_gps.csv","rb");
	if(fp!=NULL) 
	...
	fprintf(fp_log,"time,Pn,Pe,Pd,Vn,Ve,Vd,Roll,Pitch,Yaw\n");
	...
	{
		//Read one record
		fscanf(fp,"%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n",&data[log_record].time,&data[log_record].gx,&data[log_record].gy,&data[log_record].gz,&data[log_record].ax,&data[log_record].ay,&data[log_record].az,&data[log_record].mx,&data[log_record].my,&data[log_record].mz,&data[log_record].gpsn,&data[log_record].gpse,&data[log_record].gpsd,&data[log_record].gpsvn,&data[log_record].gpsve,&data[log_record].baro,&data[log_record].roll,&data[log_record].pitch,&data[log_record].yaw);
	...
	fprintf(stdout,"###### Log ekf type:\ntime,Pn,Pe,Pd,Vn,Ve,Vd,Roll,Pitch,Yaw\n");
	//caculate loop time
	clock_t start,end;  
	start = clock();
		...
		i=0;i<log_record;i++)
	{
		...
		fprintf(fp_log,"%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n",time,ekf_estimator.ekf_result.Pos_x,ekf_estimator.ekf_result.Pos_y,ekf_estimator.ekf_result.Pos_z,ekf_estimator.ekf_result.Vel_x,ekf_estimator.ekf_result.Vel_y,ekf_estimator.ekf_result.Vel_z,ekf_estimator.ekf_result.roll,ekf_estimator.ekf_result.pitch,ekf_estimator.ekf_result.yaw);
	}
	end = clock();  
