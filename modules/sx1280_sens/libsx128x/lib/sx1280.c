...
void SX1280OnDioIrq( SX1280_t *sx1280);
...
void SX1280Init( SX1280_t *sx1280, RadioCallbacks_t *callbacks )
{
    RadioCallbacks = callbacks;

	SX1280HalIoInit(sx1280);
...
RadioOperatingModes_t SX1280GetOpMode( SX1280_t *sx1280)
{
    return OperatingMode;
}

void SX1280SetSleep( SX1280_t *sx1280, SleepParams_t sleepConfig )
{
...
void SX1280SetTx( SX1280_t *sx1280, TickTime_t timeout )
{
    uint8_t buf[3];
    buf[0] = timeout.Step;
    ...
    SX1280ClearIrqStatus( sx1280, IRQ_RADIO_ALL );

    // If the radio is doing ranging operations, then apply the specific calls
    // prior to SetTx
    if( SX1280GetPacketType( sx1280 ) == PACKET_TYPE_RANGING )
    {
...
void SX1280SetRxDutyCycle( SX1280_t *sx1280, RadioTickSizes_t Step, uint16_t NbStepRx, uint16_t RxNbStepSleep )
{
    uint8_t buf[5];
...
void SX1280SetRfFrequency( SX1280_t *sx1280, uint32_t frequency )
{
    uint8_t buf[3];
    uint32_t freq = 0;
...
void SX1280SetBufferBaseAddresses( SX1280_t *sx1280, uint8_t txBaseAddress, uint8_t rxBaseAddress )
{
    uint8_t buf[2];
...
void SX1280SetPacketParams( SX1280_t *sx1280, PacketParams_t *packetParams )
{
    ...
    {
        SX1280SetPacketType( sx1280, packetParams->PacketType );
    }
...
void SX1280GetPacketStatus( SX1280_t *sx1280, PacketStatus_t *pktStatus )
{
    uint8_t status[5];
...
int8_t SX1280GetRssiInst( SX1280_t *sx1280)
{
    uint8_t raw = 0;
...
void SX1280SetDioIrqParams( SX1280_t *sx1280, uint16_t irqMask, uint16_t dio1Mask, uint16_t dio2Mask, uint16_t dio3Mask )
{
    uint8_t buf[8];
...
void SX1280ClearIrqStatus( SX1280_t *sx1280, uint16_t irq )
{
    uint8_t buf[2];
...
uint8_t SX1280GetPayload( SX1280_t *sx1280, uint8_t *buffer, uint8_t *size , uint8_t maxSize )
{
    ...
    SX1280GetRxBufferStatus( sx1280, size, &offset );
    if( *size > maxSize )
    {
        return 1;
    }
...
void SX1280SendPayload( SX1280_t *sx1280, uint8_t *payload, uint8_t size, TickTime_t timeout )
{
    SX1280SetPayload( sx1280, payload, size );
    SX1280SetTx( sx1280, timeout );
}

uint8_t SX1280SetSyncWord( SX1280_t *sx1280, uint8_t syncWordIdx, uint8_t *syncWord )
{
    uint16_t addr;
...
{
    SX1280SetBleAccessAddress( sx1280, BLE_ADVERTIZER_ACCESS_ADDRESS );
}

void SX1280SetCrcPolynomial( SX1280_t *sx1280, uint16_t polynomial )
{
    uint8_t val[2];
...
void SX1280SetWhiteningSeed( SX1280_t *sx1280, uint8_t seed )
{
    switch( SX1280GetPacketType( sx1280 ) )
    {
        case PACKET_TYPE_GFSK:
...
void SX1280SetRangingIdLength( SX1280_t *sx1280, RadioRangingIdCheckLengths_t length )
{
    switch( SX1280GetPacketType( sx1280 ) )
    {
        case PACKET_TYPE_RANGING:
...
void SX1280SetDeviceRangingAddress( SX1280_t *sx1280, uint32_t address )
{
...
void SX1280SetRangingRequestAddress( SX1280_t *sx1280, uint32_t address )
{
    uint8_t addrArray[] = { address >> 24, address >> 16, address >> 8, address };
...
double SX1280GetRangingResult( SX1280_t *sx1280, RadioRangingResultTypes_t resultType )
{
    uint32_t valLsb = 0;
    double val = 0.0;
    ...
    {
        case PACKET_TYPE_RANGING:
            SX1280SetStandby( sx1280, STDBY_XOSC );
            ...
            switch( resultType )
            {
                case RANGING_RESULT_RAW:
                    ...
                    val = ( double )SX1280complement2( valLsb, 24 ) / ( double )SX1280GetLoRaBandwidth( sx1280 ) * 36621.09375;
                    break;
...
int8_t SX1280ParseHexFileLine( SX1280_t *sx1280, char* line )
{
    ...
    if( SX1280GetHexFileLineFields( sx1280, line, bytes, &addr, &n, &code ) != 0 )
    {
        if( code == 0 )
        {
...
void SX1280SetRangingRole( SX1280_t *sx1280, RadioRangingRoles_t role )
{
    uint8_t buf[1];
...
double SX1280GetFrequencyError( SX1280_t *sx1280 )
{
    uint8_t efeRaw[3] = {0};
    uint32_t efe = 0;
    double efeHz = 0.0;
    ...
    switch( SX1280GetPacketType( sx1280 ) )
    {
        ...
        case PACKET_TYPE_RANGING:
            ...
            efeHz = 1.55 * ( double )SX1280complement2( efe, 20 ) / ( 1600.0 / ( double )SX1280GetLoRaBandwidth( sx1280 ) * 1000.0 );
            break;
...
void SX1280SetPollingMode( SX1280_t *sx1280)
{
    PollingMode = true;
}
...
double SX1280GetRangingCorrectionPerSfBwGain( SX1280_t *sx1280, const RadioLoRaSpreadingFactors_t sf, const RadioLoRaBandwidths_t bw, const int8_t gain){
...
void SX1280SetInterruptMode( SX1280_t *sx1280)
{
    PollingMode = false;
}
...
uint16_t irqRegs;
void SX1280ProcessIrqs( SX1280_t *sx1280)
{
    ...
    packetType = SX1280GetPacketType( sx1280 );
    irqRegs = SX1280GetIrqStatus( sx1280 );
    SX1280ClearIrqStatus( sx1280, IRQ_RADIO_ALL );
            ...
            switch( OperatingMode )
            {
                case MODE_RX:
                    if( ( irqRegs & IRQ_RX_DONE ) == IRQ_RX_DONE )
                    {
                        if( ( irqRegs & IRQ_CRC_ERROR ) == IRQ_CRC_ERROR )
                        {
                            ...
                            {
                                RadioCallbacks->rxError( sx1280, IRQ_CRC_ERROR_CODE );
                            }
                        }
                        else if( ( irqRegs & IRQ_SYNCWORD_ERROR ) == IRQ_SYNCWORD_ERROR )
                        {
                            if( ( RadioCallbacks != NULL ) && ( RadioCallbacks->rxError != NULL ) )
                            {
                                RadioCallbacks->rxError( sx1280, IRQ_SYNCWORD_ERROR_CODE );
                            }
                        }
                        else
                        {
                    ...
                    if( ( irqRegs & IRQ_SYNCWORD_VALID ) == IRQ_SYNCWORD_VALID )
                    {
                        ...
                        {
                            RadioCallbacks->rxSyncWordDone( sx1280 );
                        }
                    }
                    ...
                    if( ( irqRegs & IRQ_RX_TX_TIMEOUT ) == IRQ_RX_TX_TIMEOUT )
                    {
                        ...
                        {
                            RadioCallbacks->rxTimeout( sx1280 );
                        }
                    }
                ...
                case MODE_TX:
                    ...
                    if( ( irqRegs & IRQ_RX_TX_TIMEOUT ) == IRQ_RX_TX_TIMEOUT )
                    {
                        ...
                        {
                            RadioCallbacks->txTimeout( sx1280 );
                        }
                    }
                    break;
                default:
            ...
            switch( OperatingMode )
            {
                case MODE_RX:
                    ...
                    if( ( irqRegs & IRQ_HEADER_VALID ) == IRQ_HEADER_VALID )
                    {
                        ...
                        {
                            RadioCallbacks->rxHeaderDone( sx1280 );
                        }
                    }
                    if( ( irqRegs & IRQ_HEADER_ERROR ) == IRQ_HEADER_ERROR )
                    {
                        if( ( RadioCallbacks != NULL ) && ( RadioCallbacks->rxError != NULL ) )
                        {
                            RadioCallbacks->rxError( sx1280, IRQ_HEADER_ERROR_CODE );
                        }
                    }
                    if( ( irqRegs & IRQ_RX_TX_TIMEOUT ) == IRQ_RX_TX_TIMEOUT )
                    {
                    ...
                    if( ( irqRegs & IRQ_RANGING_SLAVE_REQUEST_DISCARDED ) == IRQ_RANGING_SLAVE_REQUEST_DISCARDED )
                    {
                        if( ( RadioCallbacks != NULL ) && ( RadioCallbacks->rxError != NULL ) )
                        {
                            RadioCallbacks->rxError( sx1280, IRQ_RANGING_ON_LORA_ERROR_CODE );
                        }
                    }
                    if( ( irqRegs & IRQ_TX_DONE ) == IRQ_TX_DONE )
                    {
                ...
                case MODE_CAD:
                    if( ( irqRegs & IRQ_CAD_DONE ) == IRQ_CAD_DONE )
                    {
                        if( ( irqRegs & IRQ_CAD_ACTIVITY_DETECTED ) == IRQ_CAD_ACTIVITY_DETECTED )
                        {
                            ...
                            {
                                RadioCallbacks->cadDone( sx1280, true );
                            }
                        }
                        else
                        {
                            if( ( RadioCallbacks != NULL ) && ( RadioCallbacks->cadDone != NULL ) )
                            {
                                RadioCallbacks->cadDone( sx1280, false );
                            }
                        }
                    }
            ...
            switch( OperatingMode )
            {
                ...
                case MODE_RX:
                    if( ( irqRegs & IRQ_RANGING_SLAVE_REQUEST_DISCARDED ) == IRQ_RANGING_SLAVE_REQUEST_DISCARDED )
                    {
                        ...
                        {
                            RadioCallbacks->rangingDone( sx1280, IRQ_RANGING_SLAVE_ERROR_CODE );
                        }
                    }
                    if( ( irqRegs & IRQ_RANGING_SLAVE_REQUEST_VALID ) == IRQ_RANGING_SLAVE_REQUEST_VALID )
                    {
                        if( ( RadioCallbacks != NULL ) && ( RadioCallbacks->rangingDone != NULL ) )
                        {
                            RadioCallbacks->rangingDone( sx1280, IRQ_RANGING_SLAVE_VALID_CODE );
                        }
                    }
                ...
                case MODE_TX:
                    if( ( irqRegs & IRQ_RANGING_MASTER_RESULT_TIMEOUT ) == IRQ_RANGING_MASTER_RESULT_TIMEOUT )
                    {
                        if( ( RadioCallbacks != NULL ) && ( RadioCallbacks->rangingDone != NULL ) )
                        {
                            RadioCallbacks->rangingDone( sx1280, IRQ_RANGING_MASTER_ERROR_CODE );
                        }
                    }
                    if( ( irqRegs & IRQ_RANGING_MASTER_RESULT_VALID ) == IRQ_RANGING_MASTER_RESULT_VALID )
                    {
                        if( ( RadioCallbacks != NULL ) && ( RadioCallbacks->rangingDone != NULL ) )
                        {
                            RadioCallbacks->rangingDone( sx1280, IRQ_RANGING_MASTER_VALID_CODE );
                        }
                    }
                    break;
                default:
