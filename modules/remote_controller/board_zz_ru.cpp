...
F1GPIO _dbg(GPIOB, GPIO_Pin_4);
F1GPIO _dbg2(GPIOB, GPIO_Pin_5);
...
F1GPIO power_leds[4] =
{
	F1GPIO(GPIOA, GPIO_Pin_1),
	F1GPIO(GPIOA, GPIO_Pin_2),
	F1GPIO(GPIOA, GPIO_Pin_3),
	F1GPIO(GPIOA, GPIO_Pin_4),
};
...
F1GPIO boom_led(GPIOB, GPIO_Pin_2);

F1GPIO keys[2] =
{
	F1GPIO(GPIOB, GPIO_Pin_1),
	F1GPIO(GPIOB, GPIO_Pin_3),
};

F1GPIO ants[2] =
{
	F1GPIO(GPIOA, GPIO_Pin_8),
	F1GPIO(GPIOA, GPIO_Pin_9),
};
...
int board_init()
{
	...
	boom_led.set_mode(MODE_OUT_PushPull);
	boom_led.write(false);
...
void select_ant(uint32_t *randomizer, bool tx)
{
	...
	{
		ants[i].set_mode(MODE_OUT_PushPull);
		ants[i].write(!((randomizer[tx?1:0]+i)&1));
	}
}
...
void custom_output(uint8_t * payload, int payload_size, int latency)
{
	...
	boom_led.write(key&1);
	
	pwm_init();
	TIM3->CCR3 = (key&1) ? 1000 : 2000;
}
