...
int yet_another_pilot::run_controllers()
{
	...
	attitude_controll.provide_states(euler, use_EKF ==1.0f ? &ekf_est.ekf_result.q0 : q, body_rate.array, motor_saturated ? LIMIT_ALL : LIMIT_NONE, airborne);
...
int yet_another_pilot::calculate_state()
{
	...
	TRACE("br:%.1f,%.1f,%.1f\n", body_rate.array[0] * 180 / PI, body_rate.array[1] * 180 / PI, body_rate.array[2] * 180 / PI);
...
int yet_another_pilot::handle_fifo_controll(HAL::IFIFO *fifo)
{
	if(!fifo)
		return -1;
	float q[4] = {q0,q1,q2,q3};
	int ret;
	ret = fifo->write(q,sizeof(q));
	return ret;
}
void yet_another_pilot::main_loop(void)
{
	...
	calculate_state();
	handle_fifo_controll(manager.get_FIFO("imu_FIFO"));
	handle_acrobatic();
	run_controllers();
	output();
	save_logs();
