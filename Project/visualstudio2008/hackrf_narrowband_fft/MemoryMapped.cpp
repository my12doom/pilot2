...
bool MemoryMapped::open(const std::string& filename, size_t mappedBytes, CacheHint hint)
{
  ...
  // open file
  _file = ::CreateFileA(filename.c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, winHint, NULL);
  if (!_file)
    return false;
...
bool MemoryMapped::remap(uint64_t offset, size_t mappedBytes)
{
  ...
  // get memory address
  _mappedView = ::MapViewOfFile(_mappedFile, FILE_MAP_ALL_ACCESS, offsetHigh, offsetLow, mappedBytes);
