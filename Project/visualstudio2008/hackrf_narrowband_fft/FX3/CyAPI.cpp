...
bool CCyUSBDevice::Open(UCHAR dev){
    ...
    if (CreateHandle(dev)) {
        ...
        if ((BcdUSB & BCDUSBJJMASK) == USB30MAJORVER)
        {
            if(GetInternalBosDescriptor()) // USB3.0 specific descriptor
			{
				...
				catch(char *Str)
				{
					...
					MessageBoxA(NULL,"Please correct firmware BOS descriptor table","Wrong BOS Descriptor",(UINT) NULL);
        ...
        for (int i=0; i<Configs; i++)
		{
			...
			{
				char *Str1 = Str; // just to ignore warning
				MessageBoxA(NULL,"Please correct firmware descriptor table","Wrong Device Configuration",(UINT) NULL);
				Close(); // Close the device handle.
				return false;
			}
        }
...
PUCHAR CCyUSBEndPoint::BeginDirectXfer(PUCHAR buf, LONG bufLen, OVERLAPPED *ov)
{
    ...
    PUCHAR pXmitBuf = (PUCHAR)malloc(sizeof (SINGLE_TRANSFER));//(PUCHAR)ov + sizeof(OVERLAPPED);
